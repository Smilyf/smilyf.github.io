...
//     }
// }
function addAuxiliary(element: HTMLTextAreaElement, value: string, focus_start: number, focus_end: number) {
...
function create(article_json: any, init_domain_json: any, index: string) {
    ...
    div_button1.addEventListener("click", async () => {
        ...
        let content_ = await content.text()
        sessionStorage.setItem("content", content_);
        (document.querySelector("#un_full_screen_content_left") as HTMLTextAreaElement).value = content_;
        (document.querySelector("#create-articles") as HTMLDivElement).click()
        if (sessionStorage.getItem("navigation_article") == "dynamic") {
            (document.querySelector("#drafts") as HTMLInputElement).className = "";
            (document.querySelector("#announce") as HTMLInputElement).className = "displaynone";
            (document.querySelector("#updatearticle") as HTMLInputElement).className = "";
            (document.querySelector("#cancelupdate") as HTMLInputElement).className = ""
        }
        if (sessionStorage.getItem("navigation_article") == "manuscript") {
            (document.querySelector("#drafts") as HTMLInputElement).className = "displaynone";
...
async function article_display(state: string) {
    ...
    let page_num_: number = parseInt(page_num.toString())
    if (page_num_ == 0) {
        art.innerHTML = "暂无数据！"
...
async function select(content_index: string) {
    ...
    const create_articles = (document.querySelector("#create-articles") as HTMLDivElement)
...
window.addEventListener("load", () => {
    ...
    if (sessionStorage.getItem("title") != null) {
        (document.querySelector("#title") as HTMLInputElement).value = sessionStorage.getItem("title")

    }
    if (sessionStorage.getItem("synopsis") != null) {
        (document.querySelector("#synopsis") as HTMLTextAreaElement).value = sessionStorage.getItem("synopsis")
    ...
    let synopsis = (document.querySelector("#synopsis") as HTMLTextAreaElement)
...
window.addEventListener("load", () => {
    ...
    let un_full_screen_content_left: HTMLTextAreaElement = (document.querySelector("#un_full_screen_content_left") as HTMLTextAreaElement)
...
window.addEventListener("load", () => {
    ...
    function set_scroll(e: HTMLElement) {
